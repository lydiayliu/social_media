<?php
 ...
 try{
     ...
     $bp_sql = "CREATE TABLE `BlogPhoto` ( `bpID` INT AUTO_INCREMENT NOT NULL , `accountID`INT NOT NULL , `isPhoto` BOOLEAN NOT NULL DEFAULT FALSE , `image` BLOB NULL , `text` TEXT NULL , `title` VARCHAR(50) NULL,  `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `collectionID` INT NULL , PRIMARY KEY(`bpID`), FOREIGN KEY (`accountID`) REFERENCES Account(`accountID`), FOREIGN KEY (`collectionID`) REFERENCES Collection(`collectionID`) ON DELETE CASCADE) ENGINE=InnoDB";
     $blog_sql = "CREATE TABLE `Blog` ( `blogID` INT AUTO_INCREMENT NOT NULL , `accountID`INT NOT NULL , `text` TEXT NULL , `title` VARCHAR(50) NULL,  `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY(`blogID`), FOREIGN KEY (`accountID`) REFERENCES Account(`accountID`)) ENGINE=InnoDB";
     $photo_sql = "CREATE TABLE `Photo` ( `photoID` INT AUTO_INCREMENT NOT NULL , `accountID`INT NOT NULL , `image` BLOB NULL , `title` VARCHAR(50) NULL,  `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `collectionID` INT NULL , PRIMARY KEY(`photoID`), FOREIGN KEY (`accountID`) REFERENCES Account(`accountID`), FOREIGN KEY (`collectionID`) REFERENCES Collection(`collectionID`) ON DELETE CASCADE) ENGINE=InnoDB";
     $annotation_sql = "CREATE TABLE `Annotation` ( `photoID` INT NOT NULL , `accountID`INT NOT NULL , `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , `annotation` SET('like', 'love', 'sad', 'angry','')  NOT NULL , PRIMARY KEY (`photoID`, `accountID`, `annotation`), FOREIGN KEY (`accountID`) REFERENCES Account(`accountID`), FOREIGN KEY (`photoID`) REFERENCES Photo(`photoID`) ON DELETE CASCADE) ENGINE=InnoDB";
     $comment_sql = "CREATE TABLE `Comment` ( `photoID` INT NOT NULL , `accountID` INT NOT NULL , `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ,`comment` TEXT NOT NULL , PRIMARY KEY (`photoID`, `timestamp`, `accountID`), FOREIGN KEY (`accountID`) REFERENCES Account(`accountID`), FOREIGN KEY (`photoID`) REFERENCES Photo(`photoID`) ON DELETE CASCADE) ENGINE=InnoDB";
